...
class ReadSubscription implements Subscription {
    ...
    public void onNext(long firstStreamPosition, long lastStreamPosition, Position lastAllStreamPosition, ResolvedEvent event) {
        ...
        if (!terminated.get()) {
            subscriber.onNext(new ResolvedEventWithContext(firstStreamPosition, lastStreamPosition, lastAllStreamPosition, event.getEvent(), event.getLink(), event.getPosition().orElse(null)));
