...
public class EventStoreDBClient extends EventStoreDBClientBase {
    ...
    public CompletableFuture<ReadResult> readStream(String streamName, long maxCount, ReadStreamOptions options) {
        Publisher<ResolvedEventWithContext> publisher = readStreamReactive(streamName, maxCount, options);
        return readEventsFromPublisher(publisherMap(publisher, ResolvedEvent.class::cast));
    }

    public Publisher<ResolvedEventWithContext> readStreamReactive(String streamName) {
        return this.readStreamReactive(streamName, Long.MAX_VALUE, ReadStreamOptions.get());
    }

    public Publisher<ResolvedEventWithContext> readStreamReactive(String streamName, ReadStreamOptions options) {
        return this.readStreamReactive(streamName, Long.MAX_VALUE, options);
    }

    public Publisher<ResolvedEventWithContext> readStreamReactive(String streamName, long maxCount, ReadStreamOptions options) {
        if (options == null)
    ...
    public Publisher<ResolvedEvent> readAllReactive(long maxCount, ReadAllOptions options) {
        ...
        return publisherMap(parent, ResolvedEvent.class::cast);
    }
